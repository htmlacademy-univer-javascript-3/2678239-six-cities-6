...
export default function CardsSort({onClick, selectedSort}: CardSortProps) {
  ...
  return (
    <form className="places__sorting" action="#" method="get">
      ...
      <span className="places__sorting-type" tabIndex={0} data-test-id='sort-button' onClick={() => setIsOpen(!isOpen)}>
        {SortTypeNames.get(selectedSort)}
        <svg className="places__sorting-arrow" width="7" height="4">
          <use xlinkHref="#icon-arrow-select"></use>
        </svg>
      </span>
      <ul className={`places__options places__options--custom ${isOpen ? 'places__options--opened' : ''}`} data-test-id='sort-types-list'>
